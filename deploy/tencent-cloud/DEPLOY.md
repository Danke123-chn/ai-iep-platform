...
```bash
cd /var/www/ai-iep-platform
sed 's/YOUR_DOMAIN/_/' deploy/tencent-cloud/nginx.conf | sudo tee /etc/nginx/sites-available/ai-iep-platform
sudo ln -sf /etc/nginx/sites-available/ai-iep-platform /etc/nginx/sites-enabled/
...
2. 修改 nginx 配置中的 `YOUR_DOMAIN` 为你的域名：

```bash
sed 's/YOUR_DOMAIN/app.example.com/' deploy/tencent-cloud/nginx.conf | sudo tee /etc/nginx/sites-available/ai-iep-platform
sudo ln -sf /etc/nginx/sites-available/ai-iep-platform /etc/nginx/sites-enabled/
sudo rm -f /etc/nginx/sites-enabled/default
sudo nginx -t && sudo systemctl reload nginx
```
...
- **Site URL**：`https://app.example.com`（或 `http://公网IP`）
- **Redirect URLs** 添加：
  - `https://app.example.com/auth/callback`
  - `http://公网IP/auth/callback`（若暂用 IP 测试）
...
| 登录失败 | 检查 `.env.production` 与 Supabase 回调 URL |
| 构建内存不足 | 升级套餐到 4GB，或添加 swap |
