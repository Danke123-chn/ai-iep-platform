...
create index if not exists ieps_user_id_idx on public.ieps (user_id);
create index if not exists ieps_student_id_idx on public.ieps (student_id);

-- IEP 目标表（八大领域长期目标 + 短期目标 JSON）
...
create index if not exists iep_goals_iep_id_idx on public.iep_goals (iep_id);
...
create policy "Users can view own iep goals"
  ...
  using (
    exists (
      ...
      where ieps.id = iep_goals.iep_id and ieps.user_id = auth.uid()
    )
  );
